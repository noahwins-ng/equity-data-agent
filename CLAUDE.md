...
#### Issue Lifecycle
```
/go QNT-34                # Full orchestrator: pick → implement (with WIP commits) → sanity-check (with auto-fix) → ship
...
/ship QNT-34              # Ship: squash WIPs → tick project-plan.md → PR → CI → merge → Linear Done
