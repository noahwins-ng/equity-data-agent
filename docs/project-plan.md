...
- [x] Enable run_monitoring + backfill tag concurrency in dagster.yaml — QNT-114
    - **Triggered by**: Apr 21 2026 post-incident backfill — `__ASSET_JOB` (MSFT/GOOGL/AMZN partitions) OOM'd the daemon cgroup; the AMZN run-worker was kernel-killed before emitting `RUN_FAILURE`, leaving its row in STARTED forever. Operator "Terminate" flipped it to CANCELING but never progressed (CANCELING requires the worker to ack, and the worker was dead). The ghost slot held 1 of 3 `max_concurrent_runs`, the backfill daemon kept auto-relaunching AMZN producing more ghosts, and the queue wedged ~30 min until the daemon was restarted. Root cause: Dagster has no auto-reconcile for orphaned STARTED/CANCELING runs; `run_monitoring` must be explicitly enabled. Shipped in two steps: (1) initial PR enabled `run_monitoring` + `tag_concurrency_limits`; (2) post-ship live chaos test (injected a ghost STARTED run on prod) exposed that `DefaultRunLauncher` returns `supports_check_run_worker_health = False`, so the worker-health recovery path is a no-op for us — the `MonitoringDaemon` ticked three times over 8 min and did nothing. Hotfix PR adds `max_runtime_seconds: 1800` as a timeout-based fallback: any STARTED run older than 30 min is auto-failed on the next poll. 30 min matches our real workload ceiling (sensor jobs <1 min, scheduled <5 min, max_concurrent_runs=3 backfill <10 min) and the Apr 21 incident duration. Same PR keeps `tag_concurrency_limits: dagster/backfill: 2` so a 3-partition backfill can't starve sensors. A proper launcher switch to `DockerRunLauncher` (which advertises health-check support → per-worker detection in ~2 min instead of 30 min) is tracked as a separate follow-up ticket. Complements QNT-113 (fan-out cap) and QNT-110 (launch-time retry): QNT-113 prevents new OOM fan-out, QNT-114 cleans up ghosts when OOMs still happen, QNT-110 gets a second chance at transient ones. Durable lesson captured: "run_monitoring.enabled: true" is necessary but not sufficient — the launcher must advertise `supports_check_run_worker_health` for per-worker detection; otherwise `max_runtime_seconds` is the only fallback.
