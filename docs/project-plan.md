...
- [ ] dagster-code-server memory leak mitigation: Grafana's container_memory_high alert fired continuously for ~100 MB/day steady growth toward the 2 GiB cap; investigation ruled out FD/thread leaks, our ClickHouseResource/QdrantResource code, and Dagster's own SqliteEventLogStorage (properly disposes engines), leaving a documented grpcio call-churn native-heap leak (Dagster discussion #25276, grpc/grpc#36117) tied to the 4 always-on sensors' 30s eval cadence as the leading hypothesis. Mitigation: minimum_interval_seconds=300 on 3 of 4 materialization sensors, 60s on news_raw_sensor (kept lower to protect news_embeddings_checks._COUNT_DELTA_TOLERANCE, which is calibrated on near-immediate firing). AC2/AC3 (RSS-flattening + alert-clearing) pending the 48h post-deploy observation window - QNT-472
