...
- [x] Portfolio-launch hardening: search rate limits (internal-caller exempt), token-budget usage fallback, shared-network limit copy, personal email removal (placeholder + sops), README/ADR-017 drift (2026-07-19 pre-LinkedIn readiness assessment) - QNT-388
- [x] Custom domain terminal.noahng.dev: Vercel domain + Cloudflare DNS-only CNAME, CORS origin appended via .env.sops, README/screenshots live-demo links updated; old -ynr2 URL stays as a working alias (2026-07-20 noahng.dev purchase, pre-LinkedIn launch surface) - QNT-396
- [x] Bound prod Docker build-cache by size not age: QNT-167's weekly age-based cron (--filter unused-for=168h) spared all sub-7-day cache, so ~15 GB rode permanently under CD churn; switched make prune-build-cache/-install to a daily --max-used-space=5GB size cap (Docker 29 removed --keep-storage), reclaim receipt 16->6 GB (2nd run 0 B = steady state; df over-reports vs BuildKit dedup accounting), runbook age-vs-size cross-ref (2026-07-24 prod docker df review) - QNT-405
- [x] Upgrade DeepSeek V4 Flash to the 0731 GA build (equity-agent/default + DeepEval judge, in lockstep); DeepEval RAGAS improved on every metric, golden-bench composite dropped ~8-12% (structure axis) across 3 runs, max_tokens re-derivation caught a stale narrate-vs-Thesis assumption from QNT-383 - QNT-442
- [x] Finnhub logo CDN redirect outage: staticN shard rebalancing (302 static2->static9) broke all 10 watchlist logos deterministically since httpx defaults to not following redirects; fix validates the Location header host against _FINNHUB_CDN_HOST_PATTERN before issuing the follow-up GET (not after, per review finding - following blindly then checking the response would still fire the request to a compromised host first), bounded to one hop - QNT-443
- [x] dagster-code-server memory leak mitigation: Grafana's container_memory_high alert fired continuously for ~100 MB/day steady growth toward the 2 GiB cap; investigation ruled out FD/thread leaks, our ClickHouseResource/QdrantResource code, and Dagster's own SqliteEventLogStorage (properly disposes engines), leaving a documented grpcio call-churn native-heap leak (Dagster discussion #25276, grpc/grpc#36117) tied to the 4 always-on sensors' 30s eval cadence as the leading hypothesis. Mitigation: minimum_interval_seconds=300 on 3 of 4 materialization sensors, 60s on news_raw_sensor (kept lower to protect news_embeddings_checks._COUNT_DELTA_TOLERANCE, which is calibrated on near-immediate firing). Two independent same-length post-restart windows (8h and 21h) confirmed a durable ~2.6x growth-rate reduction (~151 -> ~57 MiB/day) and the alert held clear 21+h -- a mitigation not a cure (still leaks, just ~4x slower time-to-cap); closed at 21h rather than the full 48h since both windows converged on the same slope - QNT-472
