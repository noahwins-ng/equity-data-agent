...
- [ ] Initialize monorepo with uv workspaces (4 packages)
- [ ] Set up root `pyproject.toml` with shared dev dependencies (ruff, pyright, pytest)
- [ ] Create `shared` package with `Settings`, ticker registry (`TICKERS` list + `TICKER_METADATA` dict with sector/industry per ticker), and base Pydantic schemas
- [ ] Write `docker-compose.yml` with dev/prod profiles
- [ ] Write `Dockerfile` (multi-stage: base with uv deps → dagster target, api target) — shared by dagster, dagster-daemon, and api services
- [ ] Set up ClickHouse with DDL migration scripts (raw + derived databases)
- [ ] Configure GitHub Actions for CI (lint + test) and CD (SSH deploy)
- [ ] Create `.env.example` with all required environment variables
- [ ] Verify: SSH tunnel to ClickHouse works, Dagster UI starts locally, CI pipeline passes
...
**Deliverables**:
- [ ] Implement `ohlcv_raw` Dagster asset (yfinance → ClickHouse)
...
- [ ] Implement `fundamentals` Dagster asset (yfinance → ClickHouse)
    - `StaticPartitionsDefinition` by ticker
    - Fetches all available quarterly + annual data each run
- [ ] Add Dagster schedules: daily for OHLCV (~5-6 PM ET), weekly for fundamentals
- [ ] Implement Dagster resource for ClickHouse client (shared across assets)
- [ ] Implement `make seed` (QNT-82) — quick seed script (30 days, 3 tickers) for fast local dev data setup without a full backfill
...
- [ ] `ohlcv_monthly`:
    ...
    - Downstream dependency on `ohlcv_raw` asset
- [ ] Implement `technical_indicators` Dagster assets (daily, weekly, monthly)
...
- [ ] Implement `fundamental_summary` Dagster asset (15 ratios)
...
- [ ] Add Dagster sensors to trigger downstream recomputation when raw data refreshes:
...
- [ ] Add Dagster asset checks for data quality validation (QNT-68) (e.g., no NaN close prices, volume > 0, RSI within 0-100, no future dates)
- [ ] Verify: Run full pipeline Raw → Aggregation → Indicators, spot-check calculations against external sources (e.g., TradingView)

### Phase 3 — API Layer
**Scope**: FastAPI endpoints that serve both machine-readable data (for the frontend chart) and human-readable reports (for the agent).
...
- [ ] `GET /api/v1/reports/technical/{ticker}` — formatted text report with indicator context
- [ ] `GET /api/v1/reports/fundamental/{ticker}` — formatted text report with ratio context
- [ ] `GET /api/v1/reports/news/{ticker}` — recent news summary with sentiment (returns top-N headlines + brief sentiment narrative). Sentiment is computed by FastAPI at query time via simple keyword/headline analysis (positive/negative/neutral count) — not LLM-generated. Depends on Phase 4 `news_raw` data — returns 200 with `{"report": "No news data available."}` until Phase 4 populates data.
- [ ] `GET /api/v1/reports/summary/{ticker}` — combined text overview: latest price context, RSI interpretation, trend narrative, and sector context. Sector context derived from a static mapping in `shared/tickers.py` (e.g., `TICKER_METADATA = {"NVDA": {"sector": "Technology", "industry": "Semiconductors"}, ...}`) — not fetched dynamically. Used by the agent as a quick "at a glance" tool (the frontend dashboard uses the JSON `/dashboard/summary` endpoint instead).
...
- [ ] `GET /api/v1/ohlcv/{ticker}?timeframe=daily|weekly|monthly` — returns `[{time, open, high, low, close, adj_close, volume}]` for TradingView chart rendering. `time` is an ISO date string `"YYYY-MM-DD"` (TradingView Lightweight Charts v5 accepts this natively — do not use ISO datetime with time component).
- [ ] `GET /api/v1/indicators/{ticker}?timeframe=daily|weekly|monthly` — returns `[{time, rsi_14, macd, macd_signal, macd_hist, sma_20, sma_50, ema_12, ema_26, bb_upper, bb_middle, bb_lower}]` as row-oriented time-series (each row is one date; `null` during indicator warm-up period). `time` format: `"YYYY-MM-DD"` (same as OHLCV).
    - **Warm-up periods** (number of prior `adj_close` values needed before first non-null output): RSI-14: 14, EMA-12: 12, EMA-26/MACD/MACD signal/MACD hist: 26+9=35, SMA-20/BB(20,2): 20, SMA-50: 50. All indicators are non-null from row 50 onward (0-indexed). Frontend charts should expect leading nulls and render only non-null segments.
- [ ] `GET /api/v1/fundamentals/{ticker}` — returns `{ticker, period_end, period_type, pe_ratio, ev_ebitda, ...}[]` — latest fundamental ratios as structured JSON for the ticker detail page ratios table
- [ ] `GET /api/v1/dashboard/summary` — returns `[{ticker, price, daily_change_pct, rsi_14, rsi_signal, trend_status}]` for ALL tickers in a single response. Avoids N+1 requests on dashboard load. Frontend uses this to render ticker cards with numeric values and categorical signals.
    - `price`: latest available `close` from `ohlcv_raw` (most recent trading day — real market price, NOT `adj_close`)
    - `daily_change_pct`: `(latest_close - prev_close) / prev_close * 100` — computed by FastAPI at query time (trivial presentation arithmetic, see §2.1). On weekends/holidays, shows the most recent trading day's change.
    - `rsi_signal`: `"overbought"` (RSI > 70), `"oversold"` (RSI < 30), `"neutral"` (30–70) — derived from latest `rsi_14` in `technical_indicators_daily`
    - `trend_status`: `"bullish"` (close > SMA-50), `"bearish"` (close < SMA-50), `"neutral"` (insufficient data for SMA-50 — warm-up period)

**Utility endpoints:**
- [ ] `GET /api/v1/tickers` — returns the ticker list from `shared.tickers.TICKERS` (frontend uses this to know which tickers to render)
- [ ] `GET /api/v1/health` — health check with ClickHouse + Qdrant connectivity status
...
- [ ] No API authentication in initial scope — the API is read-only and serves public market data. Auth can be added later if needed (e.g., API keys for rate limiting).
...
- [ ] Evaluate and select free news API (QNT-52 — NewsAPI.org, GNews, or RSS feeds)
- [ ] Implement `news_raw` Dagster asset (free API → `equity_raw.news_raw` in ClickHouse)
...
- [ ] Implement `news_embeddings` Dagster asset (`news_raw` → Qdrant Cloud)
...
- [ ] Add FastAPI endpoint: `GET /api/v1/search/news?ticker=NVDA&query=earnings`
    - Returns `[{headline, source, url, published_at, score}]` — top-N results ranked by cosine similarity. `score` is a float 0-1.
    - Both `ticker` and `query` are required query params. `ticker` filters Qdrant payload; `query` is embedded and used for vector search.
    - Returns empty array `[]` if Qdrant is unreachable or no news data exists (Phase 4 not deployed).
...
- [ ] Define LangGraph state schema (ticker under analysis, gathered reports, thesis draft)
- [ ] Implement tools:
    - `get_summary_report` → calls `/reports/summary/{ticker}` (quick "at a glance" — agent calls this first to decide which deeper reports to pull)
    - `get_technical_report` → calls `/reports/technical/{ticker}`
    - `get_fundamental_report` → calls `/reports/fundamental/{ticker}`
    - `get_news_report` → calls `/reports/news/{ticker}` (top-N recent headlines + sentiment summary)
    - `search_news` → calls `/search/news` (semantic search for specific topics)
- [ ] Build agent graph: plan → gather data → analyze → synthesize thesis
- [ ] Configure LiteLLM proxy via `litellm_config.yaml`:
    - **Default**: routes to Ollama Cloud (`https://ollama.com/v1`, OpenAI-compatible) via `OLLAMA_API_KEY` — no local GPU/container needed
    - **Override**: routes to Claude API via `ANTHROPIC_API_KEY` for higher quality analysis
    - Model alias: `equity-agent/default` → the LangGraph graph references only this alias, LiteLLM resolves the backend
    - Switching between Ollama Cloud and Claude is config-only (change `litellm_config.yaml`), zero agent code changes
- [ ] System prompt enforcing the "interpret, don't calculate" mandate
- [ ] Agent CLI: `python -m agent analyze NVDA` — run single-ticker analysis from terminal
- [ ] FastAPI: `POST /api/v1/agent/chat` SSE endpoint for frontend chat page
    - **Request**: `{"ticker": "NVDA", "message": "Analyze this stock"}` — stateless, single-analysis (no conversation history; each request is independent)
    - **SSE events**: `{"event": "tool_call", "data": {"tool": "get_technical_report", "ticker": "NVDA"}}` → `{"event": "thinking", "data": {"content": "..."}}` → `{"event": "thesis", "data": {"content": "# NVDA Analysis\n..."}}` → `{"event": "done"}`
    - Streams LangGraph execution: tool calls (transparency) + final thesis (markdown)
- [ ] Agent evaluation framework: define test cases (known tickers with expected signals), run agent, score thesis quality (covers all indicators, no hallucinated numbers, coherent narrative)
...
**Dependencies**: Requires Phase 3 (API endpoints to fetch data). Requires Phase 5 for agent chat page. News sidebar gracefully degrades if Phase 4 is not yet complete.
...
- [ ] Ticker detail page (`/ticker/[symbol]`) — full analysis view
    - TradingView Lightweight Charts: candlestick + volume (`GET /api/v1/ohlcv/{ticker}`). **Chart renders `adj_close` as the candlestick close value** to avoid split discontinuities (e.g., NVDA 10:1 split June 2024 is within the 2-year data window).
    - Timeframe toggle: daily / weekly / monthly (swaps chart + indicator data)
    - Technical indicator overlays: SMA, EMA, Bollinger Bands on chart; RSI, MACD as separate panes (`GET /api/v1/indicators/{ticker}`)
    - Fundamental ratios table: 15 ratios in 5 categories (`GET /api/v1/fundamentals/{ticker}`)
    - Recent news sidebar (`GET /api/v1/search/news?ticker={ticker}`) — depends on Phase 4 data; gracefully degrades to hidden/empty if no news data exists yet
- [ ] Agent chat page (`/chat`) — conversational interface
    - Calls `POST /api/v1/agent/chat` with SSE streaming
    - **No Vercel AI SDK** — use native `fetch` + `ReadableStream` to consume SSE from the POST endpoint (native `EventSource` only supports GET). Optionally add `eventsource-parser` (~2KB) for SSE line parsing.
    - Displays agent thesis with markdown rendering
    - Shows which tools the agent called (transparency)
- [ ] Generate TypeScript types from FastAPI's `/openapi.json` via `openapi-typescript` — do not handwrite types in `lib/api.ts`
    - Add `make types` command: runs `npx openapi-typescript http://localhost:8000/openapi.json -o frontend/lib/api-types.ts` (requires FastAPI running locally)
    - Re-run `make types` whenever API response schemas change — this is a manual dev step, not CI-enforced
    - Note: `POST /api/v1/agent/chat` SSE endpoint is a Phase 5 deliverable (agent package), not a Phase 6 one
- [ ] Deploy to Vercel, set `NEXT_PUBLIC_API_URL` in Vercel dashboard (not `.env` — Vercel env vars are set in project settings, e.g., `https://api.your-domain.com`)
...
- [ ] Integrate Langfuse for agent trace logging (thoughts, tool calls, latency)
- [ ] Integrate Sentry for FastAPI error tracking (`sentry-sdk[fastapi]`, uses `SENTRY_DSN` from `.env`)
- [ ] Add Dagster alerting on asset materialization failures
- [ ] Implement retry logic for flaky external API calls (yfinance, news APIs)
- [ ] Load test FastAPI endpoints (confirm response times under 10 tickers)
- [ ] Write integration tests for critical paths (ingestion → calculation → report → agent)
