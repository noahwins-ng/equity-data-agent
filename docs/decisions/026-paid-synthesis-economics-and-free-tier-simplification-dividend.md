...
| **Prompt caching** (ADR-021 #11; declined) | Declined on the free 8K-TPM per-request wall | **REVISIT / ENABLE** | Paid removes the 8K-TPM wall that made every cache-capable host `413` on a 9–12K synthesize (the sole reason ADR-021 declined). Input tokens **dominate** the bill (842K in vs 72K out — ~12:1), and the large stable prefix (system prompt + force-injected company report) is exactly what caches. This is now the single biggest cost lever. → **follow-up ticket (investigate/enable).** |
...
- **Prompt caching lands (QNT-318) and shifts the input:output economics** — the $/turn and the "input-dominated" framing here should be restated against cached-prefix pricing.
