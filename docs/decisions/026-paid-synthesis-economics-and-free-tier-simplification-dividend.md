...
| **Prompt caching** (ADR-021 #11; declined) | Declined on the free 8K-TPM per-request wall | **REVISIT → DECLINED AGAIN** ([ADR-027](027-prompt-caching-declined-again-provider-and-privacy.md)) | Paid removes the 8K-TPM wall, but QNT-318 found caching still unrealizable: on OpenRouter only the official DeepSeek provider implicit-caches `deepseek-v4-flash`, and it is excluded by our data-policy guardrail (trains on prompts — the ADR-025 hot-path boundary); the privacy-compliant providers we route to don't prefix-cache (measured `cached_tokens` 0 unpinned / flat 64 on DeepInfra). Pinning to DeepSeek also raises base input +56% ($0.09→$0.14/M), losing below a ~36% hit rate. → **declined, no config change.** |
...
- **Prompt caching becomes realizable** — QNT-318 ([ADR-027](027-prompt-caching-declined-again-provider-and-privacy.md)) declined it (no privacy-compliant OpenRouter provider caches DeepSeek V4 Flash; the one that does is data-policy-excluded). Reopen if a privacy-compliant provider adds implicit caching, our data policy is deliberately relaxed, or the primary moves to a caching-capable model — then restate the $/turn and "input-dominated" framing against cached-prefix pricing.
