...
* **One privacy-shape decision baked in**: per-IP `user_id` is `sha256(client_ip)[:12]`, never the raw IP. Matches the spirit of ADR-017 (rate-limit by IP, never store the IP). Keeps the Langfuse UI filterable without putting raw addresses in observability tooling.
