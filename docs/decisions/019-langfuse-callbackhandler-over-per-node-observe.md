...
In May 2026 the project hit Langfuse's 50k events / month free-tier ceiling. PR #259 cut volume by env-stripping Langfuse during eval bench runs and dropping `@observe` from the six tool wrappers (HTTP-to-FastAPI calls, not LLM calls). Per-chat observation count dropped from ~12 to ~6 — roughly doubling free-tier headroom under the same chat volume. The next structural lever was to align with the Langfuse-LangGraph cookbook so we could tag traces with `session_id` / `user_id` for filterable views, and drop the per-node `@observe` decorator forest in favour of one CallbackHandler at graph entry. (Sampling, the more aggressive cost lever, was considered but deferred — see Alternatives.)
...
1. `agent.tracing` shrinks to a thin client + a `make_callback_handler()` factory + re-exports of `observe`, `propagate_attributes`, `get_client` from the Langfuse SDK.
...
* **Sampling via `LANGFUSE_SAMPLE_RATE`.** Considered and prototyped during this ticket — `Langfuse(sample_rate=0.2)` would cap incoming events at ~20% of traffic. Reverted before merge: PR #259 + this migration already roughly doubled free-tier headroom (per-chat events ~12 → ~6), and dropping 80% of traces costs real debug visibility on a public demo where any specific user complaint becomes hard to reproduce. Decision: clear existing traces in the Langfuse UI to reset the monthly window, run at full fidelity, and only reintroduce sampling if the next billing window trends past 50k. The lever is a one-line addition to `_build_client` when needed; no point keeping the env var configured against a problem we may not have.
* **Self-host Langfuse on Hetzner.** Free events forever, trades infra cost for an extra container in the obs-smoke gate (~512 MB RAM, another failure surface — `feedback_health_endpoint_is_not_durability.md` is the cautionary tale). Worth doing if sampling is insufficient, not before.
...
* **Easier**: idiomatic alignment with Langfuse's documented LangGraph pattern; structured-output retries, prompt-template chains, and tool bindings are auto-traced (we currently miss those); session-id and user-id filters work in the Langfuse UI; ~130 LoC removed from the tracing layer; future engineers find the pattern in the cookbook instead of having to read our wrapper. The `Langfuse(sample_rate=...)` lever is reachable in one line if the next billing window trends past 50k.
...
* Memory: `feedback_quality_before_capacity_in_fallback.md` (selection-by-quality first, capacity second — sampling is the right capacity lever after the structural cuts in PR #259 + this migration, but only reach for it if/when actually needed)
