...
**2026-06-01 (QNT-215, gpt-oss-120b emergency fallbacks):** Added `equity-agent/fallback-cerebras-gptoss120b` after Scout in the chat fallback chain, with Groq-hosted `equity-agent/fallback-groq-gptoss120b` as the last resort. Trigger: QNT-215 dialogue-eval runs exhausted both the default Llama-3.3-70B TPD bucket and the Scout fallback bucket before the paired gate could complete. This is emergency capacity insurance, not a quality promotion. The QNT-129 bench found Groq-hosted gpt-oss-120b safe on hard contracts but too defensive for primary thesis quality, so Llama-3.3-70B remains default and Scout remains first fallback. Cerebras sits before Groq gpt-oss to provide provider-level quota diversity when Groq TPD is the failure mode; both aliases preserve the no-paid-provider chat invariant.

**2026-06-11 (QNT-223, synthesize/narrate routing moved to ADR-021):** Evaluated moving `synthesize`/`narrate` to a cache-capable gpt-oss model (Groq prefix caching) and a tail-bounding provider strategy. Outcome: synthesize/narrate **stay on `equity-agent/default` (Llama-3.3-70B)** — no Groq free-tier gpt-oss model (20b or 120b) can serve the agent's 8.4–12k-token synthesis request under Groq's 8000-TPM per-request ceiling, and Cerebras rejects the output schemas' `minItems`/`maxItems`. The per-node routing decision for these two nodes now lives in [ADR-021](021-synthesis-model-and-tail-routing.md), which also adds a proxy `request_timeout`+fallback to bound the throttled tail. ADR-011's default/override/fallback topology is otherwise unchanged.

**2026-06-11 (QNT-227, Cerebras fallback removed):** Removed `equity-agent/fallback-cerebras-gptoss120b` from every chat fallback chain (reverses the QNT-215 addition above). A controlled run confirmed Cerebras cannot serve the 9–12k-token synthesize request within the 60s client timeout — 0/4 trials, with client retries stacking to ~181s before erroring (the exact unbounded tail ADR-021 set out to kill). litellm fallbacks are recursive, so the array-bounded `ThesisPlan` schema (on `equity-agent/small`) also reached and was rejected by Cerebras. Chains are now `default → scout → groq-gptoss` and `scout → groq-gptoss`. The `groq/gpt-oss-120b` terminal still serves the small plan/classify calls and fast-fails (413) the heavy synthesize rather than hanging. Full evidence + disposition in [ADR-021's QNT-227 follow-up](021-synthesis-model-and-tail-routing.md). The `bench-cerebras-gptoss120b` alias (dialogue-eval judge + QNT-129 bench) is untouched.
