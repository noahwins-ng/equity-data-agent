...
| Cache | All server fetches use `next: { revalidate: 86_400 }`. The chart and indicator-aggregation toggles are client-side (see below) and bypass the server cache. |
...
**Cache reports in Vercel KV / Edge Config.** Overkill for our scale. ISR with `revalidate: 86_400` on the same Vercel runtime gets 99 % of the win without adding a state store to operate. *Out of scope; revisit if `revalidate` proves insufficient.*
...
- **Free-tier-friendly.** ISR + `revalidate: 86_400` means Vercel caches the rendered HTML and re-uses it across the visitor population. FastAPI sees ~1 request per ticker per day, not per navigation. Fits inside Hetzner CX41 + Vercel hobby tier comfortably -- and stays well under the Vercel free-tier ISR Writes cap, which the original 60 s value did not (QNT-166).
...
- FastAPI proves to be the bottleneck even with `revalidate: 86_400` -- step up to Vercel KV / Edge Config caching.
