...
- **Watch (deep-fallback parity)**: only the three lead providers (Novita/DeepInfra/GMICloud) are exercised here. The three deep-fallback providers (DeepSeek/Alibaba/Baidu) are **unverified for `reasoning.enabled:false` + forced structured-output parity** — the exact axis that produced the QNT-258 "DeepSeek prose `json_invalid`" regression. They only serve if all three leads are simultaneously down (rare), and `graph.py::_structured_call` fail-closes a malformed payload to a deterministic answer, so the blast radius is bounded — but if a lead-provider outage is ever observed, smoke the reached fallback against the `ThesisPlan` schema before trusting it.
